			...
			if ( 'font' == $style || 'logo' == $style || 'tagline' == $style || 'decoration' == $style ) {
				...
				if ( isset( $font->family->name ) && '' != $font->family->name ) {
					if ( isset( $font->family->fonttype ) && 'google' == $font->family->fonttype ) {
						...
						if ( ! is_array( $user_subsets ) ) {
							$user_subsets = explode( ',', $user_subsets );
						}
						$font_hash = $font->family->name . implode(',', $user_subsets);
						wp_enqueue_style( 'custom-google-fonts-' . md5($font_hash), themify_base_https_esc( 'http://fonts.googleapis.com/css' ) . '?family=' . str_replace( ' ', '+', $font->family->name ) . '&subset=' . implode(',', $user_subsets) );
					}
