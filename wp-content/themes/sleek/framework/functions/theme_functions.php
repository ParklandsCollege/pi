...
if( !function_exists( 'sleek_add_body_classes' ) ){
	function sleek_add_body_classes($classes) {
		...
		if( $theme_settings->general['post_navigation'] ){
...
if( !function_exists( 'sleek_layout_classes' ) ){
	function sleek_layout_classes(){
		...
		if( is_single() || is_page() || is_404() ){
			$layout_classes .= ' bg-content--single';
		}else{
			$layout_classes .= ' bg-content--list';
		}
...
if( !function_exists( 'sleek_get_wp_excerpt' ) ){
	function sleek_get_wp_excerpt( $length_callback = '', $use_more = true ) {
		...
		if( $use_more ){
			$more ='&nbsp;<a class="read-more" href="' . get_permalink($post->ID) . '"></a>';
		}else{
			$more = '';
		}
...
add_filter('image_send_to_editor', 'sleek_remove_thumbnail_dimensions', 10);
...
if( !function_exists( 'sleek_remove_featured_posts_from_homepage_loop' ) ){
	function sleek_remove_featured_posts_from_homepage_loop($query){
		...
		if (
			...
			$featured_cat_id = $theme_settings->posts['featured_category'];
			$featured_cat_id_minus = -1 * (int)$featured_cat_id;

			$query->set( 'cat', $featured_cat_id_minus );
		}
	}
}
...
if( !function_exists( 'sleek_wrap_oembed' ) ){
	function sleek_wrap_oembed($html, $url, $attr) {
		return '<div class="sleek-embed-container">'.$html.'</div>';
	}
}
...
/*------------------------------------------------------------
 * Backwards compatibility for Title Tag Theme Support
 *------------------------------------------------------------*/

if( !function_exists( '_wp_render_title_tag' ) ){

	// Shiv <title> to head
	add_action( 'wp_head', 'sleek_slug_render_title' );

	if( !function_exists( 'sleek_slug_render_title' ) ){
		function sleek_slug_render_title() {
			?>
	...
	// Hack <title> for homepage
	add_filter( 'wp_title', 'sleek_hack_wp_title_for_home' );

	if( !function_exists( 'sleek_hack_wp_title_for_home' ) ){
		function sleek_hack_wp_title_for_home( $title ) {
...
if( !function_exists( 'sleek_open_graph_meta' ) ){
	function sleek_open_graph_meta() {
		...
		if( is_single() && $theme_settings->general['open_graph_use'] ){
			...
			$img = wp_get_attachment_image_src( get_post_thumbnail_id(), 'xl' );
			$img = $img[0];
