...
function sleek_breadcrumbs( $post_id = NULL ) {
	$output = '';

	if ( function_exists( 'sleek_breadcrumbs_plus' ) ) {
		$output = sleek_breadcrumbs_plus( array(
				'prefix' => '<div class="breadcrumbs">',
				'suffix' => '</div>',
				'title' => false,
				'home' => __( 'Home', 'theme_frontend' ),
				'front_page' => false,
				'bold' => false,
				'blog' => __( 'Blog', 'theme_frontend' ),
				'echo' => false,
				'post_id' => $post_id
			) );
	}
 ...
 * @return string
 */
function sleek_breadcrumbs_plus( $args = '' ) {

	/* Set up the default arguments for the breadcrumb. */
	$defaults = array(
		'prefix' => '<p>',
		'suffix' => '</p>',
		'title' => __( 'You are here: ', 'theme_frontend' ),
		'home' => __( 'Home', 'theme_frontend' ),
		'separator' => '',
		'front_page' => false,
		'show_blog' => true,
	...
	if ( is_front_page() && !$args['front_page'] )
		return apply_filters( 'mm_breadcrumbs_plus', false );
	...
	$breadcrumbs = '<div class="mm-breadcrumbs">';
	...
	$breadcrumbs = apply_filters( 'mm_breadcrumbs_plus', $breadcrumbs );
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			if(is_numeric(get_query_var('w') && 0 !== get_query_var('w') ))
				$item['last'] =  sprintf( __('Weekly Archive for: "%s"','theme_frontend'),get_the_time('W'));
			elseif ( is_day() )
				$item['last'] = sprintf( __('Daily Archive for: "%s"','theme_frontend'),get_the_time('F jS, Y'));
			elseif ( is_month() )
				$item['last'] =  sprintf( __('Monthly Archive for: "%s"','theme_frontend'),get_the_time('F jS, Y'));
			elseif ( is_year() )
				$item['last'] =  sprintf(__('Yearly Archive for: "%s"','theme_frontend'),get_the_time('Y'));
		}

		else if ( is_author() )
			$item['last'] =  sprintf(__('Author Archive for: "%s"','theme_frontend'),get_the_author_meta( 'display_name', $wp_query->post->post_author ));
	}

	/* If viewing search results. */
	else if ( is_search() )
		$item['last'] =  sprintf(__('Search Results for: "%s"','theme_frontend'),stripslashes( strip_tags( get_search_query() ) ));

	/* If viewing a 404 error page. */
	else if ( is_404() )
		$item['last'] = __( 'Page Not Found', 'theme_frontend' );
...
function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() )
			$item[] = __( 'Split', 'theme_frontend' );

		elseif ( bbp_is_topic_merge() )
			$item[] = __( 'Merge', 'theme_frontend' );

		elseif ( bbp_is_topic_edit() )
			$item[] = __( 'Edit', 'theme_frontend' );
	}
