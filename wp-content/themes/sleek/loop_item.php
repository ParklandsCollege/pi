<?php
	...
	if( $post_format == 'video' || $post_format == 'audio' ){
		...
		// check if embed is external
		// needed for checking if audio is audio file with media player
		// or soundcloud embed [example]
		if( get_post_meta( get_the_ID(), 'format_embed_url', true ) ){
			if ( sleek_url_is_external( get_post_meta( get_the_ID(), 'format_embed_url', true ) ) ){
				$post_classes .= ' post--embed-external post--embed-true';
			}else{
				$post_classes .= ' post--embed-internal post--embed-true';
			}
		}else{
